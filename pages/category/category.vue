	...
	<view class="content">
		...
		<scroll-view scroll-with-animation scroll-y class="right-aside" @scroll="asideScroll" :scroll-top="navScrollTop">
			<view v-for="item in classList" :key="item._id" class="s-list" :id="'nav-'+item._id">
				...
				<view class="t-list">
					<view @click="navToList(item._id)" v-if="c.partentId === item._id" class="t-item" v-for="c in item.children"
					 :key="c._id">
						<image :src="c.logoFilePath"></image>
						<text>{{c.name}}</text>
					</view>
				</view>
			</view>
		...
		methods: {
			...
			calcSize() {
				...
				this.classList.forEach(v => {
					query.select(`#nav-${v._id}`).boundingClientRect(data => {
						v.top = data.top
					}).exec()
				})
