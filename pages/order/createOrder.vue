<template>
  <view>
    ...
    <view class="goods-section">
      <view class="g-header b-b"></view>
      <!-- 商品列表 -->
      <view class="g-item" v-for="order in orderList" :key="order._id">
        <image :src="order.imageFilePath" />
        <view class="right">
          <text class="title clamp">{{order.skuName}}</text>
          <text class="spec">{{order.goodsName}}</text>
          <view class="price-box">
            <text class="price">￥{{order.price}}</text>
            <text class="number">x {{order.num}}</text>
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="yt-list">
      <view class="yt-list-cell b-b">
        ...
        <text class="cell-tip">￥{{goodsTotalMoney}}</text>
      </view>
      <view class="yt-list-cell b-b">
        <text class="cell-tit clamp">优惠金额</text>
        <text class="cell-tip red" v-if="couponPrice>0">-￥{{couponPrice}}</text>
      </view>
      <view class="yt-list-cell b-b">
    ...
    <view class="mask" :class="maskState===0 ? 'none' : maskState===1 ? 'show' : ''" @click="toggleMask">
      <view class="mask-content" @click.stop.prevent="stopPrevent">
        <!-- 优惠券页面，仿mt -->
        <view class="coupon-item" v-for="(item,index) in couponList" :key="index" @click="onChangeCoupon(item)">
          <view class="con">
            <view class="left">
...
import { createCorder } from "@/api/user.js";
export default {
  computed: {
    ...mapState({
